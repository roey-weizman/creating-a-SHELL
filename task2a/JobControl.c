...
void freeJob(job* job_to_remove){
	...
	job_to_remove->idx=0;				/* index of current job (1 for the first) */
	job_to_remove->pgid=0; 		/* save group id of the job*/
	job_to_remove->status=0;
	free(job_to_remove);
}
...
job* initializeJob(char* cmd){
	job* myJob=malloc(sizeof(job));
	myJob->cmd=strdup(cmd);
    myJob->idx=0;				/* index of current job (1 for the first) */
	myJob->pgid=0; 		/* save group id of the job*/
	myJob->status=0; /* 1 running 0 suspendid -1 Done  TODO: see if there's a need for it print done jobs before a command is performed*/
	myJob->tmodes=(struct terminos*)malloc(sizeof(struct termios)); /* saved terminal modes */
    myJob->next=NULL;
...
job* findJobByIndex(job* job_list, int idx){
  	if(job_list==NULL)
  		perror("There is no such job with specified index");
  	else if(job_list->idx==idx)
  		return job_list;
  	else{
  		return findJobByIndex(job_list->next,idx); /*maybe *  */
  	}
}
...
void updateJobList(job **job_list, int remove_done_jobs){
	job* myList= *job_list;
	job* forPrinting=*job_list;
	while(myList!=NULL){
			while(waitpid(myList->pgid,RUNNING, WNOHANG)==0);
			myList->status=DONE;
			myList=myList->next;
	}
	if (job_to_remove->status == DONE){
		while(forPrinting!=NULL){
				if(forPrinting->status!=DONE){
					forPrinting=forPrinting->next;
					continue;
				}
		printf("[%d]\t %s \t\t %s", forPrinting->idx, statusToStr(forPrinting->status),forPrinting -> cmd); 
			
		if (forPrinting -> cmd[strlen(forPrinting -> cmd)-1]  != '\n')
				printf("\n");
			
		job* job_to_remove = forPrinting;
		forPrinting = forPrinting -> next;
		removeJob(job_list, job_to_remove);
		}
}
